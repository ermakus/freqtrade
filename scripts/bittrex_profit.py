...
def print_stat(df, idx):
    ...
    print("{0} {1: .8f} {2: 4d} {3: 4d} {4: 4d}".format(
          idx, profit, df.shape[0], buy.shape[0], sell.shape[0]))


def full_stats(filename, days):
    fh = open(filename, 'rb')
    buf = BytesIO(fh.read().decode('UTF-16').encode('UTF-8'))
    df = read_csv(buf, parse_dates=["Opened", "Closed"], index_col="Closed")
    df.sort_index(inplace=True)
    if days:
        last = df.index[-1].to_pydatetime().replace(hour=0, minute=0, second=0)
        last = last - timedelta(days=days-1)
...
def main():
    parser = argparse.ArgumentParser(
                    description='Parse bittrex orders csv and calulate daily profits')
